...
protected:
    ...
    const std::vector<std::pair<std::string, std::string>> reference_lib_options = {std::make_pair("--option_one", "option_one_description"), std::make_pair("--option_two", "option_two_description")};
    ...
    bool load_reference_plugin()
    {
        ...
        std::filesystem::path plugin_path = get_plugin_path(reference_library_name);
...
{
    TEST_START
    std::filesystem::path test_plugin_path = get_plugin_path(reference_library_name);
