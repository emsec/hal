 ...
 * are for in the tests for netlist (get_input_pins, ... )
 *
 * Functions: get_input_pins, get_output_pins, get_inout_pins
 */
TEST_F(gate_test, check_pins)
{
    TEST_START
    // Create a gate and append it to its netlist
    std::shared_ptr<netlist> nl     = create_empty_netlist();
    ...
    EXPECT_EQ(test_gate->get_inout_pins(), std::vector<std::string>({}));
