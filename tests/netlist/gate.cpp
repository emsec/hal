...
namespace hal
{
     ...
     */
    TEST_F(GateTest, check_gnd_vcc_gate_handling){TEST_START{// Mark and unmark a global vcc Gate
                                                             auto nl = test_utils::create_empty_netlist();
...
/**
     ...
     */
TEST_F(GateTest, check_get_module){TEST_START{// get the Module of a Gate (the top_module), then add it to another Module and check again
...
/**
     ...
     */
TEST_F(GateTest, check_get_grouping){TEST_START{// get the grouping of a Gate (nullptr), then add it to another grouping and check again
                                                auto nl = test_utils::create_empty_netlist();
...
/**
     ...
     */
TEST_F(GateTest, check_location_storage){TEST_START{// Create a Gate with a location and change it afterwards
                                                    auto nl = test_utils::create_empty_netlist();
...
TEST_F(GateTest, check_lut_function)
{
    ...
    }
    {
        ...
        EXPECT_EQ(lut_gate->get_boolean_function("O_LUT").get_truth_table(input_pins), std::vector<BooleanFunction::Value>(8, BooleanFunction::X));
    }
    {
        ...
        EXPECT_EQ(lut_gate->get_boolean_function("O_LUT").get_truth_table(input_pins), std::vector<BooleanFunction::Value>((1 << input_pins.size()), BooleanFunction::X));
    }
    TEST_END
}
