...
namespace hal
{
     ...
     */
    TEST_F(ModuleTest, check_constructor){TEST_START{// Creating a Module of id 123 and type "test Module"
                                                     auto nl = test_utils::create_empty_netlist();
    Module* test_module = nl->create_module(MIN_MODULE_ID + 123, "test Module", nl->get_top_module());
...
/**
     ...
     * Functions: set_name
     */
TEST_F(ModuleTest, check_set_id){TEST_START{// Set a new name for Module
                                            auto nl = test_utils::create_empty_netlist();
Module* test_module = nl->create_module(MIN_MODULE_ID + 0, "test_module", nl->get_top_module());
...
/**
     ...
     */
TEST_F(ModuleTest, check_module_type){TEST_START{// Set a new type for Module
                                                 auto nl = test_utils::create_empty_netlist();
Module* test_module = nl->create_module(MIN_MODULE_ID + 0, "test_module", nl->get_top_module());
...
/**
     ...
     */
TEST_F(ModuleTest, check_set_parent_module){TEST_START
                                            // POSITIVE
...
/**
     ...
     */
TEST_F(ModuleTest, check_contains_gate){TEST_START
                                        // POSITIVE
...
/**
     ...
     */
TEST_F(ModuleTest, check_assign_gate){TEST_START{// Add some gates to the Module
                                                 auto nl = test_utils::create_empty_netlist();
Gate* gate_0 = nl->create_gate(MIN_GATE_ID + 0, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_0");
...
Gate* gate_not_in_m = nl->create_gate(MIN_GATE_ID + 2, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_not_in_m");

// Add gate_0 and gate_1 to a Module
Module* test_module = nl->create_module(MIN_MODULE_ID + 0, "test Module", nl->get_top_module());
...
/**
     ...
     */
TEST_F(ModuleTest, check_remove_gate){TEST_START{// Delete a Gate from a Module (Gate owned by the modules)
                                                 auto nl = test_utils::create_empty_netlist();
Module* m_0  = nl->create_module(MIN_MODULE_ID + 0, "test_module", nl->get_top_module());
Gate* gate_0 = nl->create_gate(MIN_GATE_ID + 0, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_0");
m_0->assign_gate(gate_0);
...
}
{
    ...
    auto nl         = test_utils::create_empty_netlist();
    Module* m_0     = nl->create_module(MIN_MODULE_ID + 0, "test_module", nl->get_top_module());
    Module* m_other = nl->create_module(MIN_MODULE_ID + 1, "other_test_module", nl->get_top_module());
    Gate* gate_0    = nl->create_gate(MIN_GATE_ID + 0, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_0");
...
/**
     ...
     */
TEST_F(ModuleTest, check_get_gate_by_id){TEST_START
                                         // POSITIVE
                                         {// get a Gate by its id (Gate owned by Module)(not recursive)
                                          auto nl = test_utils::create_empty_netlist();
Module* m_0    = nl->create_module(MIN_MODULE_ID + 0, "test_module", nl->get_top_module());
Gate* gate_123 = nl->create_gate(MIN_GATE_ID + 123, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_123");
...
}
{
    ...
    std::set<Module*> exp_result = {};
    EXPECT_EQ(tm->get_submodules(test_utils::module_name_filter("non_existing_name"), false), exp_result);
}
}
...
TEST_F(ModuleTest, check_get_input_nets)
{
    ...
    Gate* gate_0 = nl->create_gate(MIN_GATE_ID + 0, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_0");
    Gate* gate_1 = nl->create_gate(MIN_GATE_ID + 1, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_1");
    Gate* gate_2 = nl->create_gate(MIN_GATE_ID + 2, test_utils::get_gate_type_by_name("gate_2_to_1"), "gate_2");
    Gate* gate_3 = nl->create_gate(MIN_GATE_ID + 3, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_3");
    Gate* gate_4 = nl->create_gate(MIN_GATE_ID + 4, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_4");
    Gate* gate_5 = nl->create_gate(MIN_GATE_ID + 5, test_utils::get_gate_type_by_name("gate_1_to_1"), "gate_5");
...
TEST_F(ModuleTest, check_port_names)
{
...

    {
        ...
        std::map<Net*, std::string> exp_input_port_names = {{nl->get_net_by_id(MIN_NET_ID + 13), "port_name_net_1_3"}, {nl->get_net_by_id(MIN_NET_ID + 20), "I(0)"}};
        EXPECT_EQ(m_1->get_input_port_names(), exp_input_port_names);
    }
    {
        // Get all output port names
        std::map<Net*, std::string> exp_output_port_names = {{nl->get_net_by_id(MIN_NET_ID + 045), "port_name_net_0_4_5"}, {nl->get_net_by_id(MIN_NET_ID + 78), "O(0)"}};
    ...
    m_2->assign_gate(nl->get_gate_by_id(MIN_GATE_ID + 0));
    {
        ...
        std::map<Net*, std::string> exp_input_port_names = {{nl->get_net_by_id(MIN_NET_ID + 20), "I(0)"}};
        EXPECT_EQ(m_2->get_input_port_names(), exp_input_port_names);
    }
    {
        // Get all output port names. The old ports shouldn't be contained.
        std::map<Net*, std::string> exp_output_port_names = {{nl->get_net_by_id(MIN_NET_ID + 045), "O(0)"}};
