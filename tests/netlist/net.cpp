...
TEST_F(net_test, check_set_src){
    TEST_START
    {
        ...
        bool suc                      = test_net->add_src(t_gate, "O");
        EXPECT_TRUE(suc);
    ...
    // Negative
    {
        ...
        std::shared_ptr<netlist> nl   = create_empty_netlist(MIN_NETLIST_ID+0);
        std::shared_ptr<net> test_net = nl->create_net(MIN_NET_ID+1, "test_net");
        bool suc                      = test_net->add_src(nullptr, "O");
        EXPECT_FALSE(suc);
    ...
    }
    {
        ...
        std::shared_ptr<net> test_net = nl->create_net(MIN_NET_ID+1, "test_net");
        auto t_gate_0                 = create_test_gate(nl, MIN_GATE_ID+1);
        bool suc                      = test_net->add_src(t_gate_0, "I0");    // <- input pin
        EXPECT_FALSE(suc);
        EXPECT_TRUE(is_empty(test_net->get_src()));
    }
    {
        ...
        std::shared_ptr<net> test_net = nl->create_net(MIN_NET_ID+1, "test_net");
        bool suc                      = test_net->add_src(nl->get_gate_by_id(MIN_NET_ID+1), "O");
        EXPECT_FALSE(suc);
        EXPECT_TRUE(is_empty(test_net->get_src()));
    }
    {
        ...
        bool suc = test_net->add_src(t_gate, "NEx_PIN");
    ...
    TEST_START
        {
            ...
            auto t_gate                   = create_test_gate(nl, MIN_GATE_ID+1);
            test_net->add_src(t_gate, "O");
            bool suc = test_net->remove_src(t_gate, "O");
            EXPECT_TRUE(is_empty(test_net->get_src()));
            EXPECT_TRUE(suc);
        }
        // {
...
TEST_F(net_test, check_is_a_dst){
    TEST_START
        ...
        }
        {
            ...
            test_net->add_dst(t_gate, "I2");
            EXPECT_FALSE(test_net->is_a_dst(no_dest_gate, "I2"));
        }
        {
...
TEST_F(net_test, check_get_dsts)
{
    TEST_START
        ...
        }
        {
            ...
            EXPECT_TRUE(vectors_have_same_content(test_net->get_dsts([](const auto& ep){return ep.get_gate()->get_type()->get_name() == "INV";}), dsts));
        }
    TEST_END
}
