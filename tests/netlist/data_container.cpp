...
namespace hal {
     ...
     */
    TEST_F(DataContainerTest, check_set_data) {
        TEST_START
            {
            ...
            // Negative
            {
                ...
                EXPECT_EQ(d_cont.get_data_by_key("", "key"), m_empty_pair);
            }
            {
                // Leave key empty
                NO_COUT_TEST_BLOCK;
                ...
                //EXPECT_FALSE(d_cont.data_update_notified());
                EXPECT_EQ(d_cont.get_data_by_key("category", ""), m_empty_pair);
            }
     ...
     */
    TEST_F(DataContainerTest, check_delete_data) {
        TEST_START
            {
                ...
                //EXPECT_TRUE(d_cont.data_update_notified());
                EXPECT_EQ(d_cont.get_data_by_key("category", "key"), m_empty_pair);
            }
            {
     ...
     */
    TEST_F(DataContainerTest, check_get_data_by_key) {
        TEST_START
            ...
            }
            {
                ...
                EXPECT_EQ(d_cont.get_data_by_key("category_0", "key_2"), m_empty_pair);
            }
            // NEGATIVE
            {
                // Leave category empty
     ...
     */
    TEST_F(DataContainerTest, check_get_data_keys) {
        TEST_START
            // Create a data container, filled with some entries
            TestDataContainer d_cont;
            d_cont.set_data("category_0", "key_0", "data_type_0", "value_0", false);
            ...
            std::vector<std::tuple<std::string, std::string>> keys = {
                std::make_tuple("category_0", "key_0"), std::make_tuple("category_0", "key_1"),
                std::make_tuple("category_1", "key_2"), std::make_tuple("category_1", "key_0")};

            EXPECT_TRUE(test_utils::vectors_have_same_content(d_cont.get_data_keys(), keys));
