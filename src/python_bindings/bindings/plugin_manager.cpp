...
namespace hal
{
    void plugin_manager_init(py::module& m)
    {
        ...
        py_plugin_manager.def(
            "get_plugin_instance", [](const std::string& plugin_name) -> BasePluginInterface* { return plugin_manager::get_plugin_instance<BasePluginInterface>(plugin_name, true); }, py::arg("plugin_name"), R"(
