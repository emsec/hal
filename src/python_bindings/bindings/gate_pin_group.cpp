...
namespace hal
{
    void gate_pin_group_init(py::module& m)
    {
        ...
        py_gate_pin_group.def_property_readonly("pins", [](const PinGroup<GatePin>& self) -> std::vector<GatePin*> { return self.get_pins(nullptr); }, py::return_value_policy::reference_internal, R"(
            The (ordered) pins of the pin groups.

            :type: list[hal_py.GatePin]
        )");

        py_gate_pin_group.def("get_pins", &PinGroup<GatePin>::get_pins, py::arg("filter") = nullptr, py::return_value_policy::reference_internal, R"(
