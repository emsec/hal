...
parallel_graph_layouter::parallel_graph_layouter()
{
    m_name        = "Parallel Layouter";
    m_description = "<p>A fast and simple layouting strategy, nodes are randomly place in a grid and nets are drawn as parallel lines</p>";
}

void parallel_graph_layouter::layout(graph_scene* scene)
{
    m_scene = scene;
...
void parallel_graph_layouter::reset()
{
}

void parallel_graph_layouter::handle_netlist_event(netlist_event_handler::event ev, std::shared_ptr<netlist> netlist, u32 associated_data)
{
    Q_UNUSED(ev);
    Q_UNUSED(netlist);
    Q_UNUSED(associated_data);
}
void parallel_graph_layouter::handle_gate_event(gate_event_handler::event ev, std::shared_ptr<gate> gate, u32 associated_data)
{
    Q_UNUSED(ev);
    Q_UNUSED(gate);
    Q_UNUSED(associated_data);
}
void parallel_graph_layouter::handle_net_event(net_event_handler::event ev, std::shared_ptr<net> net, u32 associated_data)
{
    Q_UNUSED(ev);
    Q_UNUSED(net);
    Q_UNUSED(associated_data);
}
void parallel_graph_layouter::handle_module_event(module_event_handler::event ev, std::shared_ptr<module> module, u32 associated_data)
{
    Q_UNUSED(ev);
    Q_UNUSED(module);
    Q_UNUSED(associated_data);
