...
void old_graph_layouter::handle_gate_name_changed_event(std::shared_ptr<gate> gate)
{
    id_gui_gate_lookup[gate->get_id()]->update_name();
    m_scene->update();
}

void old_graph_layouter::handle_module_gate_assigned(std::shared_ptr<module> module, u32 associated_data)
{
...
void old_graph_layouter::handle_module_gate_removed(std::shared_ptr<module> module, u32 associated_data)
{
    // UGLY: top module does not get a special color
    // should be: current top module, not global top module
    if (module == g_netlist->get_top_module())
