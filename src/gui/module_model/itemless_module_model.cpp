...
itemless_module_model::itemless_module_model(QObject* parent) : QAbstractItemModel(parent)
{
    ...
    // USE DESIGN AS TOP LEVEL OR MODULES ONLY ???
    m_root_item = new module_item("Root", 0);
    setupModelData();
}

itemless_module_model::~itemless_module_model()
{
    delete m_root_item;
}

QModelIndex itemless_module_model::index(int row, int column, const QModelIndex& parent) const
{
...
QModelIndex itemless_module_model::parent(const QModelIndex& index) const
{
    if (!index.isValid())
        return QModelIndex();
...
int itemless_module_model::rowCount(const QModelIndex& parent) const
{
    module_item* parent_item;
    if (parent.column() > 0)
        return 0;
...
int itemless_module_model::columnCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent)
    return 1;
}

QVariant itemless_module_model::data(const QModelIndex& index, int role) const
{
    if (!index.isValid())
    {
        return QVariant();
...
Qt::ItemFlags itemless_module_model::flags(const QModelIndex& index) const
{
    if (!index.isValid())
        return 0;
...
QVariant itemless_module_model::headerData(int section, Qt::Orientation orientation, int role) const
{
    Q_UNUSED(section)
...
void itemless_module_model::setupModelData()
{
...
QModelIndexList itemless_module_model::corresponding_indexes(const QList<u32>& gate_ids, const QList<u32>& net_ids, const QList<u32>& module_ids)
{
...
//        //check if task items are still active before deleting
//        //also delete child items if parent is deleted
