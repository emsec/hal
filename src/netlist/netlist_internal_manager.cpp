...
namespace hal
{
    ...
    std::unique_ptr<Netlist> NetlistInternalManager::create_deepcopy() const 
    {
        auto c_netlist = netlist_factory::create_netlist(m_netlist->m_gate_library);

        // manager, netlist_id, and top_module are set in the constructor 

        // copy nets
        for (const auto& net : m_netlist->m_nets) {
            c_netlist->create_net(net->m_id, net->m_name);
        }

        // copy gates
        for (const auto& gate : m_netlist->m_gates) {
            auto c_gate = c_netlist->create_gate(gate->m_id, gate->m_type, gate->m_name, gate->m_x, gate->m_y);
        
            for (const auto& in_point : gate->m_in_endpoints) {
                const auto net_id = in_point->m_net->m_id;
                auto c_net_raw = c_netlist->m_nets_map.at(net_id).get();

                c_net_raw->add_destination(c_gate, in_point->m_pin);
            }

            for (const auto& out_point : gate->m_out_endpoints) {
                const auto net_id = out_point->m_net->m_id;
                auto c_net_raw = c_netlist->m_nets_map.at(net_id).get();

                c_net_raw->add_source(c_gate, out_point->m_pin);
            }
        }

        // copy modules
        for (const auto& module : m_netlist->m_modules) {
            // ignore top module, since this is already created by the constructor
            if (module->m_id == 1) {
                continue;
            }
            std::vector<Gate*> c_gates;
            for (const auto& gate : module->m_gates) {
                // find gates of module in the copied netlist by id
                auto c_gate_raw = c_netlist->m_gates_map.at(gate->m_id).get();
                c_gates.push_back(c_gate_raw);
            }

            // create all modules with the top module as parent module and update later
            c_netlist->create_module(module->m_id, module->m_name, c_netlist->m_top_module, c_gates);
        }
        ...
        for (const auto& module : m_netlist->m_modules) {
            ...
            const auto module_id = module->m_id;
            const auto parent_id = module->m_parent->m_id;
            auto c_module_raw = c_netlist->m_modules_map.at(module_id).get();
            auto c_parent_raw = c_netlist->m_modules_map.at(parent_id).get();

            c_module_raw->set_parent_module(c_parent_raw);
        }

        // copy grougpings
        for (const auto& grouping : m_netlist->m_groupings) {
            auto c_grouping = c_netlist->create_grouping(grouping->m_id, grouping->m_name);
        
            for (const auto& module : grouping->m_modules) {
                const auto module_id = module->m_id;
                c_grouping->assign_module_by_id(module_id);
            }

            for (const auto& net : grouping->m_nets) {
                const auto net_id = net->m_id;
                c_grouping->assign_net_by_id(net_id);
            }

            for (const auto& gate : grouping->m_gates) {
                const auto gate_id = gate->m_id;
                c_grouping->assign_net_by_id(gate_id);
            }
        }

        // mark globals
        for (const auto& global_input_net : m_netlist->m_global_input_nets) {
            auto c_global_input_net = c_netlist->get_net_by_id(global_input_net->m_id);
            c_netlist->mark_global_input_net(c_global_input_net);
        }
        for (const auto& global_output_net : m_netlist->m_global_output_nets) {
            auto c_global_output_net = c_netlist->get_net_by_id(global_output_net->m_id);
            c_netlist->mark_global_output_net(c_global_output_net);
        }
        for (const auto& gnd_gate : m_netlist->m_gnd_gates) {
            auto c_gnd_gate = c_netlist->get_gate_by_id(gnd_gate->m_id);
            c_netlist->mark_gnd_gate(gnd_gate);
        }
        for (const auto& vcc_gate : m_netlist->m_vcc_gates) {
            auto c_vcc_gate = c_netlist->get_gate_by_id(vcc_gate->m_id);
            c_netlist->mark_vcc_gate(vcc_gate);
        }


        c_netlist->m_design_name = m_netlist->m_design_name;
        c_netlist->m_device_name = m_netlist->m_device_name;
        c_netlist->m_file_name = m_netlist->m_file_name;

        // update ids last, after all the creation
        c_netlist->m_next_gate_id = m_netlist->m_next_gate_id;
        c_netlist->m_used_gate_ids = m_netlist->m_used_gate_ids;
        c_netlist->m_free_gate_ids = m_netlist->m_free_gate_ids;

        c_netlist->m_next_net_id = m_netlist->m_next_net_id;
        c_netlist->m_used_net_ids = m_netlist->m_used_net_ids;
        c_netlist->m_free_net_ids = m_netlist->m_free_net_ids;

        c_netlist->m_next_module_id = m_netlist->m_next_module_id;
        c_netlist->m_used_module_ids = m_netlist->m_used_module_ids;
        c_netlist->m_free_module_ids = m_netlist->m_free_module_ids;

        c_netlist->m_next_grouping_id = m_netlist->m_next_grouping_id;
        c_netlist->m_used_grouping_ids = m_netlist->m_used_grouping_ids;
        c_netlist->m_free_grouping_ids = m_netlist->m_free_grouping_ids;
