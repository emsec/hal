...
namespace ConstantPropagation {
	...
	{
		std::vector<BooleanFunction::Value> simplified; simplified.reserve(p0.size());
	...
	{
		std::vector<BooleanFunction::Value> simplified; simplified.reserve(p.size());
	...
	std::variant<BooleanFunction, std::string> SymbolicExecution::evaluate(const BooleanFunction& function) const 
	{
	    ...
	    switch (stack.size()) 
	    {
	    	...
	    	default: ERROR("Cannot simplify '" << function.to_string() << "' (= imbalanced stack with " << stack.size() << " remaining parts).");
	    }
	}
