...
namespace {
    ...
    {
        std::vector<std::string> outputs; outputs.reserve(function.size());
    ...
    Result<Abc_Ntk_t*> translate_to_abc(const BooleanFunction& function)
    {
        ...
        auto translate = [] (const auto& node, auto&& operands, const auto& input2abc, auto network) -> Result<Abc_Obj_t*> {
            ...
            if (node.get_arity() != operands.size()) {
                return ERR("Invalid number of parameters (= " + std::to_string(operands.size()) + " for node '" + node.to_string() + "'.");
            }
            ...
            switch (node.type) {
                case BooleanFunction::NodeType::Variable: return OK(input2abc.at(node.variable));

                case BooleanFunction::NodeType::And: return OK(Abc_AigAnd((Abc_Aig_t*) network->pManFunc, operands[0], operands[1]));
                case BooleanFunction::NodeType::Not: return OK(Abc_AigXor((Abc_Aig_t*) network->pManFunc, operands[0], Abc_AigConst1(network)));
                case BooleanFunction::NodeType::Or:  return OK(Abc_AigOr((Abc_Aig_t*) network->pManFunc, operands[0], operands[1]));
                case BooleanFunction::NodeType::Xor: return OK(Abc_AigXor((Abc_Aig_t*) network->pManFunc, operands[0], operands[1]));

                default: return ERR("Not implemented reached for '" + node.to_string() + "'.");
            }
        };
        ...
        for (auto i = 0u; i < output_variables.size(); i++) {
            ...
            if (status.is_error()) {
                return ERR("Cannot slice '" + function.to_string() + "' at index " + std::to_string(i) + " (= " + status.get_error().get() + ").");
            }
            ...
            for (const auto& node: status.get().get_nodes()) {
                ...
                if (translation.is_ok()) {
                    ...
                    return ERR("Cannot translate '" + node.to_string() + "' to ABC (= " + translation.get_error().get());
                }
            }

            // (4) check that we have exactely one remaining entry on the stack
            if (stack.size() != 1) {
                return ERR("Cannot translate '" + function.to_string() + "' (= " + std::to_string(stack.size()) + " elements on the stack).");
            }
        ...
        if (!Abc_NtkCheck(network)) {
            ...
            return ERR("Cannot translate '" + function.to_string() + "' as AbcNtkCheck() failed.");
        }
    ...
    {    
        ...
        for (int i = 0; i < network->vObjs->nSize; i++) {
            ...
            if (node->fPersist) {
                return ERR("Cannot handle case as 'node->fPersist' is set within ABC context.");
            }
            ...
            const auto lhs = std::string(Io_WriteVerilogGetName(Abc_ObjName(
    ...
    Result<BooleanFunction> Simplification::abc_simplification(const BooleanFunction& function)
    {
        ...
        auto status = translate_to_abc(function)
            .map<std::monostate>([] (auto network) { return simplify(network); });

        if (status.is_error()) {
            return ERR("Cannot translate & simplify '" + function.to_string() + "' with ABC (= " + status.get_error().get() + ").");
        }

        // (3) translate the ABC graph back into a Boolean function
        auto translated_function = translate_from_abc()
            .map<BooleanFunction>([&function] (auto verilog) { return translate_from_verilog(verilog, function); });
