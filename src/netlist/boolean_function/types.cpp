...
namespace ModelParser 
{
	...
	const auto NameAction = [] (const auto& ctx) {
		parser_context.current_name = _attr(ctx);
	};
	// Action to store a translated size in the parser context.
	const auto SortAction = [] (const auto& ctx) {
	 	parser_context.current_size = boost::fusion::at_c<0>(_attr(ctx));
	};
	// Action to store a translated binary value in the parser context.
	const auto BinaryValueAction = [] (const auto& ctx) { 
		parser_context.current_value = "0b" + _attr(ctx);
	};
	// Action to store a translated hexadecimal value in the parser context.
	const auto HexValueAction = [] (const auto& ctx) { 
		parser_context.current_value = "0x" + _attr(ctx);
	};

	// Action to store a translated signal assignment in the parser context.
	const auto SignalAssigmentAction = [] (const auto& /* ctx */) { 

		const auto value = (parser_context.current_value.at(1) == 'b') 
			? strtoull(parser_context.current_value.substr(2, parser_context.current_value.length() - 2).c_str(), nullptr, 2)
    ...
    template<>
    std::vector<std::string> EnumStrings<SolverType>::data = {"Z3", "Boolector", "Unknown"};
	...
	Constraint::Constraint(BooleanFunction _lhs, BooleanFunction _rhs) :
		lhs(_lhs), rhs(_rhs) {}
	...
	template<>
    std::vector<std::string> EnumStrings<ResultType>::data = {"sat", "unsat", "unknown"};

    Model::Model(std::map<std::string, std::tuple<u64, u16>> _model) :
    	model(_model) {}
	...
	{
		auto s = std::accumulate(model.model.begin(), model.model.end(), std::string(),
	...
	std::tuple<bool, Model> Model::parse(std::string s, const SolverType& type) 
	{
		...
		switch (ok && (iter == s.end()))
		{
			...
			case false: return {false, Model()};
		}
	} 

	Result::Result(ResultType _type, std::optional<Model> _model) :
