...
bool hdl_parser_verilog_old::parse_instance(const std::string& instance, const std::string& type, const int line)
{
    ...
    auto new_gate = m_netlist->create_gate(m_netlist->get_unique_gate_id(), it->second, name);
...
bool hdl_parser_verilog_old::parse_assign(const std::string& token, const int line)
{
    ...
    {
        auto lib = m_netlist->get_gate_library()->get_gate_types();
        auto it  = lib.find("BUF");
        if (it == lib.end())
        {
            log_error("hdl_parser", "Cannot parse direct net assignment since no type 'BUF' is available in the gate library.");
            return false;
        }

        std::string gate_name = nets_lhs[i] + "_" + "buffer";
        auto new_gate         = m_netlist->create_gate(m_netlist->get_unique_gate_id(), it->second, gate_name);    //Hardcoded for UNISIM lib Very hacky bad bad bad
