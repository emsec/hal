...
bool program_arguments::set_option(const std::string& flag, const std::set<std::string>& equivalent_flags, const std::vector<std::string>& parameters)
{
    ...
    {
        log_error(
            "core", "Given flags ({}) are parts of {} different options.", core_utils::join(", ", std::vector<std::string>(option_flags.begin(), option_flags.end())), options_containing_flags.size());
        return false;
    }
