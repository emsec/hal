    ...
    bool VerilatorEngine::setSimulationInput(SimulationInput* simInput)
    {
        ...
        for (const auto& sim_event : simInput->get_simulation_net_events()) {
            ...
            for (const auto& [net, boolean_value] : sim_event) {
                log_info("verilator_simulator", "net: {}", net->get_name());
                ...
                default:
                    log_error("verilator_simulator", "cannot assign value '{}' to net '{}' at {}ps, only supporting 1 and 0, aborting...", BooleanFunction::to_string(boolean_value), net->get_name(), duration);
                    return false;
                    break;
                }
        ...
        utils::replace(testbench_cpp, std::string("<insert_trace_here>"), simulation_data.str());

        // // write all files
        // std::ofstream makefile_file(m_simulator_dir / "Makefile");
        ...
        // std::ofstream testbench_clock_h_file(m_simulator_dir / "tbclock.h");
