    ...
    std::unique_ptr<GateTypeComponent> GateLibraryWizard::setComponents()
    {
        ...
        for (GateTypeProperty prop : generalInfoPage->getProperties())
        {
            ...
            else if (prop == GateTypeProperty::ff)
            {
                ...
                if (next_state_res.is_ok())
                {
                    next_state_bf = next_state_res.get();
                }
                if (clock_bf_res.is_ok())
                {
                ...
                if (async_reset_res.is_ok())
                {
                    async_reset = async_reset_res.get();
                }
                ...
                if (async_set_res.is_ok())
                {
                    async_set = async_set_res.get();
                }
            ...
            else if (prop == GateTypeProperty::latch)
            {
                ...
                if (data_in_res.is_ok())
                {
                    latch_component->set_data_in_function(data_in_res.get());
                }
                if (enable_res.is_ok())
                {
                    latch_component->set_enable_function(enable_res.get());
                }
    ...
    int GateLibraryWizard::nextId() const
    {
        ...
        switch (currentId())
        {
            case GeneralInfo:
                return Pin;
            case Pin:
                if (properties.contains(GateTypeProperty::ff) || properties.contains(GateTypeProperty::latch))
                {
                    return State;
                }
                else if (properties.contains(GateTypeProperty::c_lut))
                {
                    return LUT;
                }
                else if (properties.contains(GateTypeProperty::ram))
                {
                    return RAM;
                }
                return BoolFunc;
            case State:
                if (properties.contains(GateTypeProperty::ff))
                {
                    return FlipFlop;
                }
                else if (properties.contains(GateTypeProperty::latch))
                {
                    return Latch;
                }
                else if (properties.contains(GateTypeProperty::c_lut))
                ...
                return BoolFunc;
            case FlipFlop:
                if (properties.contains(GateTypeProperty::latch))
                {
                    return Latch;
                }
                else if (properties.contains(GateTypeProperty::c_lut))
            ...
            case Latch:
                ...
                return Init;
            case LUT:
                if (properties.contains(GateTypeProperty::ram))
                {
                    return RAM;
                }
                return Init;
