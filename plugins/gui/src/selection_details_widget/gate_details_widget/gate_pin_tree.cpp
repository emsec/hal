...
namespace hal
{
    ...
    GatePinTree::GatePinTree(QWidget* parent) : SizeAdjustableTreeView(parent), mPinModel(new GatePinsTreeModel(this)), mGateID(-1), mClearSelection(false)
    {
    ...
    void GatePinTree::handleContextMenuRequested(const QPoint& pos)
    {
        ...
        menu.addAction("Extract name as plain text", [clickedItem]() { QApplication::clipboard()->setText(clickedItem->getData(GatePinsTreeModel::sNameColumn).toString()); });

        menu.addAction("Extract direction as plain text", [clickedItem]() { QApplication::clipboard()->setText(clickedItem->getData(GatePinsTreeModel::sDirectionColumn).toString()); });

        menu.addAction("Extract type as plain text", [clickedItem]() { QApplication::clipboard()->setText(clickedItem->getData(GatePinsTreeModel::sTypeColumn).toString()); });
    ...
    void GatePinTree::buildPythonMenuForPin(QMenu& menu, TreeItem* clickedPinItem)
    {
        ...
        menu.addAction(QIcon(":/icons/python"), "Extract net(s) as python code", [pythonCommandNetIds]() { QApplication::clipboard()->setText(pythonCommandNetIds); });
        ...
        menu.addAction(QIcon(":/icons/python"), "Extract pin direction as python code", [pythonCommandDirection]() { QApplication::clipboard()->setText(pythonCommandDirection); });
        ...
        menu.addAction(QIcon(":/icons/python"), "Extract pin type as python code", [pythonCommandType]() { QApplication::clipboard()->setText(pythonCommandType); });
    }
    ...
    void GatePinTree::buildPythonMenuForPinGroup(QMenu& menu, TreeItem* clickedPinIGrouptem)
    {
        ...
        menu.addAction(QIcon(":/icons/python"), "Extract pingroup as python list", [pythonList]() { QApplication::clipboard()->setText(pythonList); });
        ...
        {
            QString pythonCommandGroupDirection = PyCodeProvider::pyCodeGateTypePinDirection(mPinModel->getCurrentGateID(), firstPinItemOfGroup->getData(GatePinsTreeModel::sNameColumn).toString());
            menu.addAction(
                QIcon(":/icons/python"), "Extract direction of pingroup as python code", [pythonCommandGroupDirection]() { QApplication::clipboard()->setText(pythonCommandGroupDirection); });

            QString pythonCommandGroupType = PyCodeProvider::pyCodeGateTypePinType(mPinModel->getCurrentGateID(), firstPinItemOfGroup->getData(GatePinsTreeModel::sNameColumn).toString());
            menu.addAction(QIcon(":/icons/python"), "Extract type of pingroup as python code", [pythonCommandGroupType]() { QApplication::clipboard()->setText(pythonCommandGroupType); });
        }
    }
    ...
    void GatePinTree::handleNavigationJumpRequested(const Node& origin, const u32 via_net, const QSet<u32>& to_gates, const QSet<u32>& to_modules)
    {
        ...
        if (to_gates.size() == 1)
        {
            ...
            for (auto pin : pins)
            {
                if (isNavDirLeft)
                {
                    if (auto res = g->get_fan_out_net(pin); res.is_ok() && res.get() == n)
                    {
                        gSelectionRelay->setFocus(SelectionRelay::ItemType::Gate, g->get_id(), SelectionRelay::Subfocus::Right, index);
                        break;
                    }
                }
                else
                {
                    if (auto res = g->get_fan_in_net(pin); res.is_ok() && res.get() == n)
                    {
                        gSelectionRelay->setFocus(SelectionRelay::ItemType::Gate, g->get_id(), SelectionRelay::Subfocus::Left, index);
                        break;
                    }
                }
                index++;
