...
namespace hal
{
    ...
    void GateDetailsTabWidget::setupBooleanFunctionTables(Gate* gate, GateDetailsTabWidget::GateTypeCategory gateTypeCategory)
    {
        ...
        std::unordered_map<std::string, BooleanFunction> allBfs;
        if (auto res = gate->get_boolean_functions(false); res.is_ok())
        {
            allBfs = res.get();
        }
        else
        ...
        {
            case GateDetailsTabWidget::GateTypeCategory::lut: {
                std::vector<std::string> lutPins = gate->get_type()->get_pin_names([](const GatePin* p) { return p->get_type() == PinType::lut; });
                ...
                if (lutPins.size() > 0)
                {
                    ...
                    const std::string& outPin = lutPins.front();

                    // Fill the LUT truth table
                    if (auto res = gate->get_boolean_function(outPin); res.is_ok())
                    {
                        mLutTable->setBooleanFunction(res.get(), QString::fromStdString(outPin));
                    }
                }
                break;
            }
