...
namespace hal
{
    ...
    GuiPluginManager::GuiPluginManager(QWidget* parent) : QWidget(parent), mDefaultTextColor(Qt::lightGray), mHilightTextColor(Qt::yellow), mHilightBackgroundColor(Qt::black)
    {
        ...
        QLabel* ltext[4] = {new QLabel("Load HAL plugin and dependencies (if any)", this),
    ...
    void GuiPluginManager::addPluginActions(QMenu* menu) const
    {
        ...
        for (auto it = guiExtensionMap.constBegin(); it != guiExtensionMap.constEnd(); ++it)
        {
            ...
            menu->addAction(label, [label, geif, menu]() {
                PluginParameterDialog ppd(label, geif, menu);
                ppd.exec();
            });
            hasEntries = true;
        }
    ...
    void GuiPluginManager::repolish()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                case 0:
                    tmpIcon = gui_utility::getStyledSvgIcon(loadIconStyle(), loadIconPath());
                    break;
                case 1:
                    tmpIcon = gui_utility::getStyledSvgIcon(unloadIconStyle(), unloadIconPath());
                    break;
                case 2:
                    tmpIcon = gui_utility::getStyledSvgIcon(cliIconStyle(), cliIconPath());
                    break;
                case 3:
                    tmpIcon = gui_utility::getStyledSvgIcon(guiIconEnabledStyle(), guiIconEnabledPath());
                    break;
            }
    ...
    {
        GuiPluginEntry::GuiExtensionState newState = GuiPluginEntry::NotAnExtension;
        GuiExtensionInterface* geif                = getGuiExtensions().value(pluginName);
        if (geif)
        {
            geif->set_contribution_enabled(!geif->is_contribution_enabled());
            newState = geif->is_contribution_enabled() ? GuiPluginEntry::Enabled : GuiPluginEntry::Disabled;
        }
    ...
    void GuiPluginManager::addPluginSubmenus(QMenu* contextMenu, Netlist* netlist, const std::vector<u32>& modules, const std::vector<u32>& gates, const std::vector<u32>& nets)
    {
        ...
        for (auto it = guiExtensionMap.constBegin(); it != guiExtensionMap.constEnd(); ++it)
        {
            ...
            for (ContextMenuContribution cmc : contribution)
            {
                ...
                connect(act, &QAction::triggered, contextMenu, [cmc, netlist, modules, gates, nets]() { cmc.mContributer->execute_function(cmc.mTagname, netlist, modules, gates, nets); });
            }
        }
    }

    //_________________VIEW_______________________________
    GuiPluginView::GuiPluginView(QWidget* parent) : QTableView(parent)
    {
        setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    }

    //_________________TABLE______________________________
    GuiPluginTable::GuiPluginTable(GuiPluginManager* parent) : QAbstractTableModel(parent), mPluginMgr(parent)
    {
    ...
    void GuiPluginTable::populateTable(bool refresh)
    {
        ...
        for (std::filesystem::path plugDir : utils::get_plugin_directories())
        {
            ...
            for (QFileInfo info : dir.entryInfoList())
            {
                ...
                if (!plugin_manager::has_valid_file_extension(info.absoluteFilePath().toStdString()))
                {
                    continue;
                }
                ...
                else if (gpe->requestLoad())
                {
                    ...
                    if (!bpif)
                    {
                        gpe->mLoadState = GuiPluginEntry::NotAPlugin;    // however, load failed
                ...
                if (!bpif)
                {
                    ...
                    gpe->mLoadState = GuiPluginEntry::NotAPlugin;
                    continue;
                }
    ...
    void GuiPluginTable::changeState(const QString& pluginName, GuiPluginEntry::LoadState state)
    {
        int irow = mLookup.value(pluginName, -1);
        if (irow < 0 || irow >= mEntries.size())
        {
            return;
        }
        GuiPluginEntry* gpe = mEntries.at(irow);
        gpe->mLoadState     = state;
        if (state >= GuiPluginEntry::AutoLoad)
        {
            GuiPluginEntry::GuiExtensionState newGuiExtensionState = GuiPluginEntry::NotAnExtension;
    ...
    QString SupportedFileFormats::toFileDialog(bool addHalFormat) const
    {
        ...
        for (auto it = pluginMap.constBegin(); it != pluginMap.constEnd(); ++it)
        {
            ...
            QString fileFmt = (re.indexIn(label) < 0) ? label.remove(QChar(':')) : re.cap(1) + QString(" files ");
    ...
    void GuiPluginTable::loadFeature(FacExtensionInterface::Feature ft, const QString& extension)
    {
        for (GuiPluginEntry* gpe : mEntries)
        {
            if (gpe->isLoaded() || gpe->mFeature != ft)
            {
                continue;
            }
            ...
            if (bpif)
            {
                gpe->updateFromLoaded(bpif, false);
            }
        }
    }
    ...
    BasePluginInterface* GuiPluginTable::load(const QString& pluginName, const QString& path) const
    {
        ...
        {
            log_warning("gui", "Error loading plugin '{}' from location '{}'", pnam, path.toStdString());
            return nullptr;
        }
    ...
    QMap<QString, GuiExtensionInterface*> GuiPluginManager::getGuiExtensions()
    {
        ...
        for (const std::string& pluginName : plugin_manager::get_plugin_names())
        {
            ...
            for (AbstractExtensionInterface* aeif : bpif->get_extensions())
            {
                ...
                if (geif)
                {
                    retval.insert(QString::fromStdString(pluginName), geif);
                }
            }
        }
        return retval;
    ...
    void GuiPluginTable::handleButtonPressed(const QModelIndex& buttonIndex)
    {
        ...
        if (gpe->isLoaded())
        {
            ...
            if (need.isEmpty())
            {
                ...
                {
                    QMessageBox::warning(qApp->activeWindow(), "Unload failed", QString("Unload of plugin %1\nrefused by plugin_manager").arg(gpe->mName));
                }
            }
            else
            {
                QMessageBox::warning(qApp->activeWindow(), "Unload failed", QString("Cannot unload plugin %1\nwhich is required by %2").arg(gpe->mName).arg(need.join(',')));
            }
        }
        else
        {
            ...
            {
                QMessageBox::warning(qApp->activeWindow(), "Load failed", QString("Could not load plugin %1\nplease check plugin_manager log").arg(gpe->mName));
            }
        }
        if (success)
        {
    ...
    GuiPluginEntry::GuiExtensionState GuiPluginTable::guiExtensionState(const QModelIndex& index) const
    {
        if (index.row() >= mEntries.size())
        {
            return GuiPluginEntry::Unknown;
        }
        return mEntries.at(index.row())->mGuiExtensionState;
    }

    void GuiPluginTable::setGuiExtensionState(const QString& pluginName, GuiPluginEntry::GuiExtensionState state)
    {
        int irow = mLookup.value(pluginName, -1);
        if (irow < 0 || irow >= mEntries.size())
        {
    ...
    GuiPluginEntry::GuiPluginEntry(const QFileInfo& info)
        : mLoadState(NotAPlugin), mName(info.baseName()), mFilePath(info.absoluteFilePath()), mFileModified(info.lastModified()), mFeature(FacExtensionInterface::FacUnknown), mUserInterface(false),
          mGuiExtensionState(Unknown), mFileFound(true)
    {
        ;
    }
    ...
    QVariant GuiPluginEntry::data(int icol) const
    {
        switch (icol)
        {
            ...
            case 3:
                return mFileModified.toString("dd.MM.yy hh:mm");
            case 4:
                return mDependencies.join(' ');
            case 5: {
                switch (mFeature)
                {
                    case FacExtensionInterface::FacNetlistParser:
                        return "Netlist parser";
                    case FacExtensionInterface::FacNetlistWriter:
                        return "Netlist writer";
                    case FacExtensionInterface::FacGatelibParser:
                        return "Gate library parser";
                    case FacExtensionInterface::FacGatelibWriter:
                        return "Gate library writer";
                    default:
                        break;
                }
                if (mUserInterface)
                {
                    return "HAL user interface";
                }
                return "Other HAL plugin";
            }
            case 6:
                return mFeatureArguments.join(' ');
            case 7:
                return mGuiExtensionState;
            case 8:
                return mCliOptions;
            case 9:
                if (!isLoaded())
            ...
            case 10:
                return (int)mLoadState;
        }
        return QVariant();
    }

    GuiPluginEntry::GuiExtensionState GuiPluginEntry::enforceGuiExtensionState(GuiExtensionInterface* geif) const
    {
        if (!geif)
        {
        ...
        return (geif->is_contribution_enabled() ? Enabled : Disabled);
    }

    void GuiPluginEntry::persist(QSettings* settings) const
    {
        settings->setValue("state", (int)mLoadState);
    ...
    GuiPluginEntry::GuiPluginEntry(const QSettings* settings) : mFileFound(false)
    {
    ...
    {
        mLoadState   = isUser ? UserLoad : AutoLoad;
        QString desc = QString::fromStdString(bpif->get_name());
        if (desc != mName)
        {
            mDescription = desc;
        }
        if (!bpif->get_description().empty())
        {
            mDescription = QString::fromStdString(bpif->get_description());
        }
        ...
        GuiExtensionState newGuiExtensionState = Unknown;    // not initialized
        for (AbstractExtensionInterface* aeif : bpif->get_extensions())
        {
        ...
        if (!newGuiExtensionState)
        {
            mGuiExtensionState = NotAnExtension;    // no GUI extension
        }

        mUserInterface = (dynamic_cast<const UIPluginInterface*>(bpif) != nullptr);
        if (modified.isValid())
        {
            mFileModified = modified;
        }
    }

    //------------------------------------------------
    GuiPluginDelegate::GuiPluginDelegate(QObject* parent) : QItemDelegate(parent)
    {
    ...
    void GuiPluginDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        bool drawIcon                               = true;
        GuiPluginEntry::GuiExtensionState iconState = GuiPluginEntry::Unknown;
        switch (index.column())
        {
            case 7:
                ...
                if (!iconState)
                {
                    ...
                    {
                        iconState = GuiPluginEntry::Unknown;
                    }
                    else if (!geif)
                    {
                        iconState = GuiPluginEntry::NotAnExtension;
                    }
                    else if (geif->is_contribution_enabled())
                    {
                        iconState = GuiPluginEntry::Enabled;
                    }
                    else
                    {
                        iconState = GuiPluginEntry::Disabled;
                    }
                }

                switch (iconState)
                {
                    case GuiPluginEntry::Unknown: {
                        drawIcon = false;
                        break;
                    }
                    case GuiPluginEntry::NotAnExtension:
                        drawIcon = false;
                        break;
                    case GuiPluginEntry::Disabled:
                        button.icon = mIconDisableGuiContribution;
                        break;
                    case GuiPluginEntry::Enabled:
