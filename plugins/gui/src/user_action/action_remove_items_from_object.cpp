...
namespace hal
{
    ActionRemoveItemsFromObjectFactory::ActionRemoveItemsFromObjectFactory() : UserActionFactory("RemoveItemsFromObject")
    {
        ;
    }
    ...
    bool ActionRemoveItemsFromObject::exec()
    {
        ...
        switch (mObject.type())
        {
            case UserActionObjectType::Context:
                ...
                if (ctx)
                {
                    ActionAddItemsToObject* undo = new ActionAddItemsToObject(mModules, mGates, mNets);
