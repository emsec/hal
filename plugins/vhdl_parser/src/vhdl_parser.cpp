    ...
    Result<std::unique_ptr<Netlist>> VHDLParser::instantiate(const GateLibrary* gate_library)
    {
            ...
            if (!m_zero_net->get_destinations().empty())
            {
                ...
                if (auto res = m_zero_net->add_source(gnd, output_pin); res.is_error())
                {
                    return ERR_APPEND(res.get_error(),
                                      "could not instantiate VHDL netlist '" + m_path.string() + "' with gate library '" + gate_library->get_name() + "': could not add source to GND gate");
                }
            }
            else
            {
        ...
        if (m_netlist->get_vcc_gates().empty())
        {
            if (!m_one_net->get_destinations().empty())
            {
                ...
                {
                    return ERR_APPEND(res.get_error(),
                                      "could not instantiate VHDL netlist '" + m_path.string() + "' with gate library '" + gate_library->get_name() + "': could not add source to VCC gate");
                }
            }
            else
            {
        ...
        for (const ci_string& inst_identifier : vhdl_entity.m_instances)
        {
            ...
            else if (const auto gate_type_it = m_gate_types.find(inst_type); gate_type_it != m_gate_types.end())
            {
                ...
                if (const auto inst_it = vhdl_entity.m_expanded_gate_assignments.find(inst_identifier); inst_it != vhdl_entity.m_expanded_gate_assignments.end())
                {
                    ...
                    {
                        pin_to_direction[core_strings::to<ci_string>(pin)] = direction;
                    }
