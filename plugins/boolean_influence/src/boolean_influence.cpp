...
#include "boolean_influence/evaluation_engines/deterministic.h"
#include "boolean_influence/evaluation_engines/optimized_bitsliced.h"
#include "boolean_influence/evaluation_engines/probabilistic.h"
...
namespace hal
{
    namespace boolean_influence
    {
        ...
        Result<std::unordered_map<std::string, double>> get_boolean_influence_bitsliced(const z3::expr& expr, const u32 num_evaluations, const u32 bias)
        {
            const auto to_replacement_var = [](const u32 var_idx) -> std::string { return "var_" + std::to_string(var_idx); };
            ...
            for (const auto& var : replacement_vars)
            {
                ...
                const std::string run_command = program_name + " " + std::to_string(idx) + " 2>&1";
        ...
        Result<std::map<Net*, double>> get_boolean_influences_of_subcircuit_bitsliced(const std::vector<Gate*>& gates, const Net* start_net, const u32 num_evaluations, const u32 bias)
        {
            for (const auto* gate : gates)
            {
            ...
            // Generate Boolean influences
            const auto inf_res = get_boolean_influence_bitsliced(func, num_evaluations, bias);
            if (inf_res.is_error())
            {
                return ERR_APPEND(inf_res.get_error(),
        ...
        Result<std::map<Net*, double>> get_boolean_influences_of_gate_bitsliced(const Gate* gate, const u32 num_evaluations, const u32 bias)
        {
            if (!gate->get_type()->has_property(GateTypeProperty::ff))
            {
                return ERR("unable to get Boolean influence for gate " + gate->get_name() + " with ID " + std::to_string(gate->get_id()) + ": can only handle flip-flops but found gate type "
            ...
            if (data_net == nullptr)
            {
                return ERR("unable to get Boolean influence for gate " + gate->get_name() + " with ID " + std::to_string(gate->get_id()) + ": failed to find net at data pin " + data_pin->get_name());
            }
            ...
            // Generate Boolean influences
            const auto inf_res = get_boolean_influences_of_subcircuit_bitsliced(function_gates, data_net, num_evaluations, bias);
            if (inf_res.is_error())
            {
                return ERR_APPEND(inf_res.get_error(),
                                  "unable to get Boolean influence for gate " + gate->get_name() + " with ID " + std::to_string(gate->get_id()) + ": failed to get Boolean influence for data net "
