...
namespace hal
{
    namespace z3_utils
    {
        BooleanFunction SubgraphFunctionGenerator::get_function_of_gate(const Gate* gate, const GatePin* out_pin)
        {
            ...
            BooleanFunction bf;
            if (auto res = gate->get_boolean_function(out_pin); res.is_error())
            {
                log_error(
                    "z3_utils", "error encountered while getting Boolean function '{}' of gate '{}' with ID {}:\n{}", out_pin->get_name(), gate->get_name(), gate->get_id(), res.get_error().get());
            ...
            while (true)
            {
                ...
                for (auto const& output_pin : output_pins_that_are_also_function_inputs)
                {
                    ...
                    {
                        log_error(
                            "z3_utils", "error encountered while getting Boolean function '{}' of gate '{}' with ID {}:\n{}", output_pin, gate->get_name(), gate->get_id(), res.get_error().get());
                        return BooleanFunction();
                    }
                    else
                    {
            ...
            {
                if (auto res = gate->get_fan_in_net(input_pin); res.is_error())
                {
                ...
                {
                    bf = bf.substitute(input_pin->get_name(), std::to_string(res.get()->get_id()));
                }
            }
        ...
        z3::expr RecursiveSubgraphFunctionGenerator::get_function_of_net(const Net* net, z3::context& ctx, const std::vector<Gate*>& subgraph_gates)
        {
            ...
            {
                GatePin* pin;
                if (auto res = src->get_type()->get_pin_by_name(pin_name); res.is_error())
                {
                    log_error("z3_utils", "error encountered while getting pin of gate '{}' with ID {} by name '{}':\n{}", src->get_name(), src->get_id(), pin_name, res.get_error().get());
                }
                else
                {
                    pin = res.get();
                }
                if (auto res = src->get_fan_in_net(pin); res.is_error())
                {
                    log_error("z3_utils", "error encountered while getting fan-in net of gate '{}' with ID {} at pin '{}':\n{}", src->get_name(), src->get_id(), pin_name, res.get_error().get());
                }
                else
                {
                    pin_to_expr.insert({pin->get_name(), get_function_of_net(res.get(), ctx, subgraph_gates)});
                }
            }
