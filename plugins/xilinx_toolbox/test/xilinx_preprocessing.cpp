...
namespace hal
{
    ...
    TEST_F(XilinxPreprocessingTest, check_split_luts_vector_skips_on_error)
    {
        TEST_START
        {
            // A non-LUT6_2 gate should be skipped; the valid gate in the same batch must still be split.
            auto  nl  = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_luts_vector_all_wrong_type)
    {
        TEST_START
        {
            // All gates invalid — should return OK(0), not an error.
            auto  nl  = make_netlist();
            Gate* buf = nl->create_gate(m_buf_type, "buf");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16e_depth1)
    {
        TEST_START
        {
            // A3:A0 = 0000 → select_value = 0 → depth 1 (one FF)
            auto nl        = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16e_depth5)
    {
        TEST_START
        {
            // A3:A0 = 0100 → select_value = 4 → depth 5 (five FFs: ff_0 .. ff_4)
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16e_max_depth)
    {
        TEST_START
        {
            // A3:A0 = 1111 → select_value = 15 → depth 16 (FFs ff_0 .. ff_15)
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16e_chain_connections)
    {
        TEST_START
        {
            // Verify that FFs are chained correctly: ff_0.D = D, ff_i.D = ff_{i-1}.Q for i>0
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16e_clk_ce_shared)
    {
        TEST_START
        {
            // All FFs must be connected to the same CLK and CE nets as the original gate
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_wrong_type)
    {
        TEST_START
        {
            auto  nl  = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_unconnected_addr_pin)
    {
        TEST_START
        {
            // An address pin with no net must produce an error, not a silent treat-as-0
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_nonconstant_addr_pin)
    {
        TEST_START
        {
            // An address pin driven by a non-constant net must produce an error
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = make_srl16e(nl.get(), "srl");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_registers_vector_skips_on_error)
    {
        TEST_START
        {
            // A gate with a non-constant address pin should be skipped;
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_module_assignment)
    {
        TEST_START
        {
            auto nl         = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srl16)
    {
        TEST_START
        {
            // SRL16 should be handled identically to SRL16E
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srlc16e_no_cascade)
    {
        TEST_START
        {
            // When Q15 has no consumers it behaves like SRL16E
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srlc16e_cascade)
    {
        TEST_START
        {
            // When Q15 is used, all 16 stages must be created; Q15 sources ff_15
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = nl->create_gate(m_srlc16e_type, "srl");
            test_utils::connect_global_in(nl.get(), srl, "CLK");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srlc32e_no_cascade)
    {
        TEST_START
        {
            // Q31 has no consumers: only select_value+1 FFs are created
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srlc32e_cascade)
    {
        TEST_START
        {
            // Q31 used: all 32 FFs must be created
            auto nl         = make_netlist();
            auto [gnd, vcc] = make_constant_nets(nl.get());
            Gate* srl       = nl->create_gate(m_srlc32e_type, "srl");
            test_utils::connect_global_in(nl.get(), srl, "CLK");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_srlc32e_both_at_max)
    {
        TEST_START
        {
            // select_value = 31 = max_depth: Q and Q31 tap the same physical stage.
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_registers_vector)
    {
        TEST_START
        {
            auto nl          = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_registers_vector_wrong_type)
    {
        TEST_START
        {
            // Wrong-type gate is skipped with a warning; the call succeeds with count 0.
            auto  nl  = make_netlist();
            Gate* buf = nl->create_gate(m_buf_type, "buf");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_registers_all_types)
    {
        TEST_START
        {
            auto nl         = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_registers_none_present)
    {
        TEST_START
        {
            auto nl  = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_lut_create_module)
    {
        TEST_START
        {
            auto nl  = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_lut_create_module_parent_preserved)
    {
        TEST_START
        {
            // When create_module=true the new module must be a child of the original gate's parent.
            auto nl   = make_netlist();
            Gate* g   = make_lut6_2(nl.get(), "myLUT");
            add_sink(nl.get(), g, "O6");
    ...
    TEST_F(XilinxPreprocessingTest, check_split_lut_no_create_module_default)
    {
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_create_module)
    {
        TEST_START
        {
            auto nl        = make_netlist();
    ...
    TEST_F(XilinxPreprocessingTest, check_split_shift_register_create_module_parent_preserved)
    {
        TEST_START
        {
            auto nl         = make_netlist();
