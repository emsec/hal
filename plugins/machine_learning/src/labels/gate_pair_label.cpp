...
#include "machine_learning/utils/json.hpp"
...
namespace hal
{
    namespace machine_learning
    {
        namespace gate_pair_label
        {
            namespace
            {
                MultiBitInformation get_multi_bit_information(LabelContext& lc)
                {
                    ...
                    for (const auto g : lc.nl->get_gates([](const Gate* g_i) { return g_i->get_type()->has_property(GateTypeProperty::ff); }))
                    {
                        ...
                        for (const auto& [name, index, direction, _origin] : index_information)
                        {
            ...
            // std::vector<std::pair<const Gate*, const Gate*>> SharedSignalGroup::calculate_gate_pairs(LabelContext& lc, const Netlist* nl) const
            // {
            //     return {};
            // };

            // std::pair<std::vector<std::pair<const Gate*, const Gate*>>, std::vector<u32>> SharedSignalGroup::calculate_label(LabelContext& lc, const Gate* g_a, const Gate* g_b) const
            // {
            //     return {};
            // }
            ...
            {
                lc.mbi          = get_multi_bit_information(lc);
                const auto& mbi = lc.mbi.value();
                ...
                for (const auto& g : lc.nl->get_gates([](const Gate* g_i) { return g_i->get_type()->has_property(GateTypeProperty::ff); }))
                {
                    ...
                    for (u32 i = 0; i < neg_count; i++)
                    {
                        ...
                        for (u32 idx = start; idx < start + lc.nl->get_gates().size(); idx++)
                        {
                            ...
                            if (pos_gates.find(g_i) == pos_gates.end())
                            {
                                pairs.push_back({g, g_i});
                                labels.push_back({{0}});
                                break;
            ...
            /*
            std::vector<std::pair<const Gate*, const Gate*>> SharedConnection::calculate_gate_pairs(LabelContext& lc, const Netlist* nl) const
            {
                std::vector<std::pair<const Gate*, const Gate*>> pairs;
                ...
                for (const auto& g : nl->get_gates())
                {
                    ...
                    for (u32 i = 0; i < neg_count; i++)
                    {
                        const u32 start = std::rand() % nl->get_gates().size();
                        for (u32 idx = start; idx < start + nl->get_gates().size(); idx++)
                        {
                            const auto g_i = nl->get_gates().at(idx % nl->get_gates().size());
            ...
            */
            ...
            {
                // const auto l = SharedConnection();
                const auto l = SharedSignalGroup();
                auto lc      = LabelContext(nl);

                const auto [pairs, labels] = l.calculate_labels(lc);
