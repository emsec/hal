...
namespace hal
{
    namespace gate_library_specific_utils
    {
        ...
        std::unordered_set<std::string> UtilsiCE40Ultra::get_set_ports(Gate* sg) const
        {
            if (!is_sequential(sg))
            {
                log_error("gl specifics", "gate is not sequential: {}, type: {}", sg->get_name(), sg->get_type()->get_name());
        ...
        std::unordered_set<std::string> UtilsiCE40Ultra::get_regular_outputs(Gate* sg) const 
        {
            if (!is_sequential(sg))
            {
                log_error("gl specifics", "gate is not sequential: {}, type: {}", sg->get_name(), sg->get_type()->get_name());
        ...
        std::unordered_set<std::string> UtilsiCE40Ultra::get_negated_outputs(Gate* sg) const 
        {
            if (!is_sequential(sg))
            {
                log_error("gl specifics", "gate is not sequential: {}, type: {}", sg->get_name(), sg->get_type()->get_name());
