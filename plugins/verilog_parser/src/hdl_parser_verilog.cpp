#include "verilog_parser/hdl_parser_verilog.h"
...
namespace hal
{
    ...
    bool HDLParserVerilog::parse_entity(std::map<std::string, std::string>& attributes)
    {
        ...
        VerilogEntity e(line_number, entity_name);
    ...
    bool HDLParserVerilog::parse_port_list(VerilogEntity& e, std::set<std::string>& port_names)
    {
        ...
        while (ports_str.remaining() > 0)
        {
            ...
            if (next_token == "input" || next_token == "output" || next_token == "inout")
            {
                while (ports_str.remaining() > 0)
                {
                    PinDirection direction = enum_from_string<PinDirection>(next_token.string, PinDirection::none);
                    ...
                    do
                    {
                        ...
                        VerilogSignal p(next_token.number, next_token.string, ranges);
    ...
    bool HDLParserVerilog::parse_port_assign(VerilogEntity& e, VerilogInstance& inst)
    {
        ...
        while (port_str.remaining() > 0)
        {
            ...
            if (right_str.size() != 0)
            {
                const VerilogSignal s(left_str.number, left_str.string, {}, false, false);
                const auto right_parts = get_assignment_signals(e, right_str, true);
    ...
    std::map<std::string, VerilogSignal> HDLParserVerilog::parse_signal_list()
    {
    ...
    std::optional<std::pair<std::vector<VerilogSignal>, i32>> HDLParserVerilog::get_assignment_signals(VerilogEntity& e, TokenStream<std::string>& signal_str, bool allow_numerics)
    {
        // PARSE ASSIGNMENT
    ...
    void HDLParserVerilog::expand_signal_recursively(std::vector<std::string>& expanded_signal, const std::string& current_signal, const std::vector<std::vector<u32>>& ranges, u32 dimension) const
    {
    ...
    std::vector<std::string> HDLParserVerilog::expand_signal_vector(const std::vector<VerilogSignal>& signals, bool allow_binary) const
    {
        std::vector<std::string> res;
