...
private:
    ...
    static std::map<std::string, std::shared_ptr<bdd>> bdd_generator_mycell(std::shared_ptr<gate> g, std::map<std::string, std::shared_ptr<bdd>>& input_pin_type_to_bdd);
    static bool bdd_availability_tester_mycell(std::shared_ptr<gate> g);
