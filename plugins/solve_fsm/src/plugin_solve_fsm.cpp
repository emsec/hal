...
namespace hal
{
    ...
    std::map<u64, std::vector<u64>> SolveFsmPlugin::solve_fsm_brute_force(Netlist* nl, const std::vector<Gate*> state_reg, const std::vector<Gate*> transition_logic, const std::string graph_path)
    {
        ...
        for (const auto& ff : state_reg)
        {
            ...
            if (bf.is_empty())
            {
                bf = std::get<BooleanFunction>(BooleanFunction::from(std::to_string(input_net->get_id())));
            }
            ...
            for (const auto& id_str : bf.get_variable_names())
            {
                ...
                {
                    external_ids_to_func.insert({id, std::get<BooleanFunction>(BooleanFunction::from(id_str))});
            ...
            for (const auto& [out, in] : output_net_to_input_net)
            {
                ...
                BooleanFunction from = std::get<BooleanFunction>(BooleanFunction::from(std::to_string(out->get_id())));
                BooleanFunction to   = std::get<BooleanFunction>(BooleanFunction::from(std::to_string(in->get_id())));
