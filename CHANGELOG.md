...
## [Unreleased]
* suppress gate coordinates when adding to cone view
* fixed crash related to GraphicsScene destructor
* added support for multiple base types for a single instance of class `GateType`
  * added function `has_base_type` to class `GateType`
  * changed function `get_base_type` of class `GateType` to `get_base_types`
  * changed HGL gate libraries to support multiple base types
